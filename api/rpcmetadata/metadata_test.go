...
func TestServerContext(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "kratos",
			args: args{context.Background(), Metadata{"hello": {"kratos"}, "hello0": {"https://go-kratos.dev"}}},
		},
		{
			name: "hello",
			args: args{context.Background(), Metadata{"hello": {"kratos"}, "hello1": {"https://go-kratos.dev"}}},
		},
		{
			name: "hello",
			args: args{context.Background(), Metadata{"hello": {"ServerAuthCheck"}, "hello2": {"###$$$%%%^^&&**(())__"}}},
		},
	}
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fmt.Println(i, m["hello"], m[fmt.Sprintf("hello%v", i)])
			if !ok {
